    ...
    <div class="container">
        ...
        @else
            <div class="row pt-5 mt-5 mt-lg-1">
                ...
                @foreach ($futureWorks as $futureWork)
                    <div class="col-12 col-lg-4 col-md-6 pt-3">
                        <a href="/futureWorkDetail/{{ $futureWork->id }}">
                            <div class="card bg-dark card-shadow plan-card">
                                <img class="card-img-top plan-card-image"
                                    src="/img/design/{{ $futureWork->thumbnail ? $futureWork->thumbnail : 'no_image.jpeg' }}"
                                    alt="Card image cap">
                                <div class="card-body plan-card-body bg-dark">
                                    <p class="text-white font-weight-bolder h5">
                                        {{ App::getLocale() == 'la' ? $futureWork->name : (App::getLocale() == 'en' ? ($futureWork->en_name ? $futureWork->en_name : $futureWork->name) : ($futureWork->cn_name ? $futureWork->cn_name : $futureWork->name)) }}
                                    </p>
                                </div>
                            </div>
                        </a>
